...
- **Font choice affects the PDF text layer, not just looks.** `-apple-system`/`BlinkMacSystemFont` (San Francisco) renders fine but produces a broken `ToUnicode` map in WebKit's PDF output: glyphs look right but copy/paste and text extraction return garbage (lowercase Cyrillic `к` → `ĸ`, etc.). Both themes therefore lead with `Helvetica Neue` and must **not** put `-apple-system`/`BlinkMacSystemFont` first in any `font-family`. Monospace SF (`SF Mono`/`SFMono-Regular`) is unaffected. Verify the text layer (not just the visual) after font changes — e.g. extract with PDFKit (`PDFDocument(url:).string`).
- **Inline emphasis uses flanking guards.** The `**`/`__`/`~~` regexes in `inlineMD` carry `(?![\s…])`/`(?<![\s…])` lookarounds so a bare run of markers (a `____________` signature line, `****`, `~~~~`) stays literal instead of collapsing into mangled empty tags. `__` additionally guards `(?<!\w)`/`(?!\w)` so intraword underscores (`snake_case`) aren't emphasized. Keep these guards when touching emphasis.
